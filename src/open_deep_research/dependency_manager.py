...
class DependencyManager:
    ...
    def _initialize_dependencies(self):
        ...
        dependencies = {
            ...
            SearchProvider.AZURE: DependencyInfo(
                ...
                required_env_vars=["AZURE_AI_SEARCH_ENDPOINT", "AZURE_AI_SEARCH_INDEX_NAME", "AZURE_AI_SEARCH_API_KEY"]
            ),
    ...
    def get_installation_commands_for_multiple(self, providers: List[SearchProvider]) -> str:
    ...
    def check_provider_health(self, provider: SearchProvider) -> HealthStatus:
        ...
        if not self.is_provider_available(provider):
            return HealthStatus(False, "Provider not installed", 
        ...
        if info.required_env_vars:
            ...
            if missing_vars:
                return HealthStatus(False, "Missing API keys", 
        ...
        if not self.is_provider_available(provider):
            ...
            if info:
                raise ProviderNotInstalledError(
                    info.name,
                    self.get_installation_command(provider),
                    info.description
                )
            else:
        ...
        for provider in SearchProvider:
            ...
            if dep_info:
                if self.is_provider_available(provider):
                    health = self.check_provider_health(provider)
                    if health.healthy:
                        report_lines.append(f"✓ {dep_info.name}: Installed and configured")
                    else:
                        error_msg = health.details or health.error or "Configuration issue"
                        report_lines.append(f"⚠️ {dep_info.name}: Installed but not configured ({error_msg})")
                else:
                    ...
                    report_lines.append(f"  - To install: {self.get_installation_command(provider)}")
