...
class SearchProviderBase(ABC):
    ...
    def validate_params(self, **kwargs) -> Dict[str, Any]:
        ...
        if missing:
            raise ValueError(f"Missing required parameters for {self.provider.value}: {missing}")
...
class UnifiedSearchInterface:
    ...
    async def search(self, provider: SearchProvider, queries: List[str], **kwargs) -> List[SearchResult]:
        """Execute search using the specified provider."""
        if provider not in self.providers:
            raise NotImplementedError(f"Provider {provider.value} not yet implemented in unified interface")
