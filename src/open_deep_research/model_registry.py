...
def get_supported_models(role: ModelRole | None = None, *, require_tool_choice: bool | None = None) -> List[str]:
    ...
    for model_id, caps in MODEL_CAPABILITIES.items():
        if role and role not in caps.get("recommended_roles", []):
            continue
        if require_tool_choice is not None and caps.get("supports_tool_choice", False) != require_tool_choice:
            continue
...
def get_available_model_combos() -> Dict[str, ModelCombo]:
    ...
    def combo_is_available(combo: ModelCombo) -> bool:
        required_providers = [get_provider(combo["planner"]), get_provider(combo["writer"])]
        if combo["summarizer"]:
...
def get_available_models(role: ModelRole | None = None, *, require_tool_choice: bool | None = None) -> List[str]:
