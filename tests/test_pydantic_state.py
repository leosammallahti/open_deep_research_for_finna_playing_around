...
def test_deep_research_state_with_data():
    ...
    messages = cast(
        "list[AnyMessage]", [HumanMessage(content="What is the capital of France?")]
    )
    state = DeepResearchState(
        topic="A test topic", sections=sections, messages=messages
    )
...
def test_model_copy_aggregates_list_fields():
    ...
    state = DeepResearchState()
    state2 = state.model_copy(update={"messages": [SystemMessage(content="first")]})
    ...
    state3 = state2.model_copy(update={"messages": [SystemMessage(content="second")]})
...
def test_model_copy_aggregates_str_field():
    ...
    # Current behavior: string replacement
