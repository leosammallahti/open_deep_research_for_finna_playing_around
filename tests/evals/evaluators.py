    ...
    eval_result = cast(
        ...
        eval_model.with_structured_output(OverallQualityScore).invoke(
            [
                {
                    "role": "system",
                    "content": OVERALL_QUALITY_PROMPT.replace(
                        "{today}", get_today_str()
                    ),
                },
    ...
    eval_result = cast(
        ...
        eval_model.with_structured_output(RelevanceScore).invoke(
            [
                {
                    "role": "system",
                    "content": RELEVANCE_PROMPT.replace("{today}", get_today_str()),
                },
                {"role": "user", "content": user_input_content},
            ]
        ),
    ...
    eval_result = cast(
        ...
        eval_model.with_structured_output(StructureScore).invoke(
            [
                {
                    "role": "system",
                    "content": STRUCTURE_PROMPT.replace("{today}", get_today_str()),
                },
                {"role": "user", "content": user_input_content},
            ]
        ),
...
def eval_groundedness(inputs: dict, outputs: dict):
    ...
    user_input_content = (
        GROUNDEDNESS_PROMPT.replace("{context}", context)
