...
if ENV_LOAD_ERROR:
    ...
    st.info("💡 **How to fix**: Create a `.env` file in the project root with your API keys in UTF-8 encoding, or set environment variables manually.")
...
st.markdown("**AI-powered research assistant** that helps you create comprehensive reports on any topic")
...
with st.sidebar:
    ...
    default_search_provider = next(
        (provider for provider in available_providers if provider.value == default_search_api), 
    ...
    search_provider_api = next(
        (provider.value for provider, name in PROVIDER_DISPLAY_NAMES.items() if name == search_provider_name),
        "duckduckgo"
    )
    ...
    if not available_combos:
        st.error("❌ No model combinations available. Please check your API keys in the .env file.")
        st.info("Required API keys: ANTHROPIC_API_KEY, OPENAI_API_KEY, or DEEPSEEK_API_KEY")
        st.stop()
    
    # Predefined Model Combos
    combo_options = {key: combo['display_name'] for key, combo in available_combos.items()}
    ...
    if selected_combo_key == "custom":
        ...
        if not planner_model_options:
            st.error("❌ No planner models available. Please configure API keys for at least one model provider.")
            st.stop()
        
        if not writer_model_options:
            st.error("❌ No writer models available. Please configure API keys for at least one model provider.")
            st.stop()
        ...
        if not supports_tool_choice(researcher_model):
            st.warning(f"**Warning:** The selected writer model (`{researcher_model}`) does not support tool calling. This may lead to errors or poor performance as it cannot reliably generate structured output.")

        # Optional summarizer drop-down if users want custom summarization model
        with st.expander("📝 Summarisation Model (optional)"):
            summarizer_model_options = get_available_models("summarizer") or writer_model_options
            default_summarizer = os.environ.get("SUMMARIZER_MODEL", summarizer_model_options[0])
        ...
        st.caption(f"Using **{combo['planner']}** for planning and **{combo['writer']}** for writing.")
...
if st.session_state.research_in_progress:
        ...
        config = {
            "configurable": {
                "search_api": SearchAPI(search_provider_api) if search_provider_api else SearchAPI.DUCKDUCKGO,
        ...
        with progress_container:
            ...
            async def run_research():
                ...
                try:
                    ...
                    # Execute the graph with proper input format for graph.py
            ...
            try:
                ...
                try:
                    ...
                    error_msg = str(e)
                    if "cannot schedule new futures after shutdown" in error_msg or \
                ...
                if "context length" in error_msg.lower():
                    st.error("❌ **Context Length Error**: The research topic is too complex for the selected model. Try:")
                    st.error("• Using a model with larger context (e.g., Claude 3.5 Sonnet)")
                    ...
                    st.error("❌ **Rate Limit Error**: The API rate limit was exceeded. Try:")
                    st.error("• Waiting 5-10 minutes before retrying")
                    st.error("• Using a different model provider (switch from OpenAI to Anthropic or vice versa)")
                    st.error("• Reducing the number of search queries per section")
                    st.error("• Using a model with higher rate limits")
                    st.info("💡 **Tip**: OpenAI has stricter rate limits than Anthropic for some models.")
                    ...
                    st.error("❌ **API Overloaded**: The AI service is currently overloaded. Try:")
                    st.error("• Waiting 1-2 minutes before retrying")
                    st.error("• Using a different AI model (e.g., switching from Claude to GPT)")
                    st.error("• Reducing the complexity of your research topic")
                    st.error("• Trying again during off-peak hours")
                    st.info("💡 **Tip**: The app automatically retries with backoff delays, but high demand can still cause failures.")
                elif "cannot schedule new futures after shutdown" in error_msg.lower():
                    st.error("❌ **System Shutdown Error**: The research was interrupted during execution. Try:")
                    st.error("• Restarting the Streamlit app")
                    st.error("• Using a simpler research topic")
                    st.error("• Avoiding rapid start/stop operations")
                    st.info("💡 **Tip**: This usually happens when research is stopped abruptly. The app is designed to handle this gracefully.")
                else:
